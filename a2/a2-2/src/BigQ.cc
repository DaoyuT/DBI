...
void * BigQ :: WorkerThread(void) {
	...
	cout<<"tempfile created in "<<tempfilename<<endl;
...
void BigQ::PhaseTwo(){
	if(totalruns > RUN_THRESHOLD){
		cout<<"Merging runs using priority queue"<<endl;
		MergeRunsPriorityQueue();
	}else{
		cout<<"Merging runs using linear scan"<<endl;
		MergeRunsLinearScan();
	}
}
...
bool CompareRunRecord::operator()(const RunRecord &rr1, const RunRecord &rr2){
		//cout<<"fetching records from run records"<<endl;
		//Record r1, r2;
		//rr1.GetRecord(&r1);
		//rr2.GetRecord(&r2);
		//cout<<"comparing run records"<<endl;
